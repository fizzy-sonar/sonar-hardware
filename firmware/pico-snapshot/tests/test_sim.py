...
from simulate import decimate_channel, make_payload, make_snapshot, power_at_tone, CLOCK_HZ, TONE_HZ
...
def main() -> None:
    ...
    assert power_at_tone(samples, TONE_HZ, CLOCK_HZ / 24) > power_at_tone(samples, 45_000, CLOCK_HZ / 24) * 10
    bad = bytearray(make_snapshot(payload)); bad[-1] ^= 1
    try:
        decode(bytes(bad))
