...
def decode(blob: bytes) -> Snapshot:
    ...
    magic, version, header_len, flags, capture_id, first, clock, channels, lines, frames, payload_len, payload_crc, header_crc, reserved = HEADER.unpack(blob[:HEADER_BYTES])
    if (magic, version, header_len, flags, channels, lines, reserved) != (b"SNP1", 1, 48, 0, 24, 12, 0):
...
def main() -> None:
    ...
    parser.add_argument("input", type=Path, help="CDC capture bytes (one complete SNP1 frame)")
    parser.add_argument("output", type=Path, help="validated raw SNP1 output")
    args = parser.parse_args()
    snapshot = decode(args.input.read_bytes())
    args.output.write_bytes(args.input.read_bytes())
    print(f"valid SNP1 capture={snapshot.capture_id} frames={snapshot.frames} clock={snapshot.clock_hz} Hz")
