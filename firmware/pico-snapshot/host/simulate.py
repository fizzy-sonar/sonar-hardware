    ...
    for n in range(frames):
        ...
        for line in range(12):
            for edge in range(2):
                ...
                value = 0.5 + 0.43 * math.sin(2.0 * math.pi * TONE_HZ * sample_time + channel * 0.07)
...
def make_snapshot(payload: bytes, capture_id: int = 7) -> bytes:
    ...
    prefix = HEADER.pack(b"SNP1", 1, 48, 0, capture_id, 0, CLOCK_HZ, 24, 12, frames, len(payload), zlib.crc32(payload) & 0xFFFFFFFF, 0, 0)
...
def power_at_tone(samples: list[float], frequency: float, sample_rate: float) -> float:
    re = sum(x * math.cos(2 * math.pi * frequency * n / sample_rate) for n, x in enumerate(samples))
    im = sum(x * math.sin(2 * math.pi * frequency * n / sample_rate) for n, x in enumerate(samples))
    return re * re + im * im
...
def main() -> None:
    ...
    print(f"channel 0 {TONE_HZ} Hz tone/off-tone power ratio: {10 * math.log10(tone / off_tone):.1f} dB")
