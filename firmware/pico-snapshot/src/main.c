...
#endif
static int dma_chan, sm; static volatile int completed = -1; static int active; static uint64_t logical_frame;
static void set_clock(uint32_t hz,bool on){pio_sm_set_enabled(pio0,sm,false);pio_sm_set_clkdiv(pio0,sm,sonar_pio_divider(clock_get_hz(clk_sys),hz));if(on)pio_sm_set_enabled(pio0,sm,true);}
...
static void startup_mics(void){set_clock(PDM_STANDARD_HZ,true);sleep_ms(PDM_STANDARD_MS);set_clock(PDM_STANDARD_HZ,false);sleep_us(2);set_clock(PDM_ULTRASONIC_HZ,true);sleep_ms(PDM_SETTLE_MS);logical_frame=PDM_DISCARD_FRAMES;pio_sm_set_enabled(pio0,sm,false);}
...
int main(void) {
    ...
    while (true) { tud_task(); int ch=getchar_timeout_us(0); if(ch=='C' && completed<0) arm_capture(); if(completed>=0){int i=completed; completed=-1; pio_sm_set_enabled(pio0,sm,false); sonar_repack_pio_words(window[i],dma_words[i],WINDOW_FRAMES); cdc_write_snapshot(window[i],WINDOW_FRAMES); logical_frame+=WINDOW_FRAMES;} }
}
